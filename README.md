...
2. Copy it into Archi's `dropins/` directory:
   ...
   **macOS**
   ```bash
   cp za.co.jesseleresche.archi.mcp-2.1.1.jar /Applications/Archi.app/Contents/Eclipse/dropins/
   ```

   **Linux**
   ```bash
   cp za.co.jesseleresche.archi.mcp-2.1.1.jar /opt/Archi/dropins/
   ```

   **Windows** (PowerShell)
   ```powershell
   Copy-Item za.co.jesseleresche.archi.mcp-2.1.1.jar "C:\Program Files\Archi\dropins\"
   ```
   ...
   > an old one and your upgrade won't take effect. Only `za.co.jesseleresche.archi.mcp-2.1.1.jar`
...
```
za.co.jesseleresche.archi.mcp/target/za.co.jesseleresche.archi.mcp-2.1.1.jar
```
